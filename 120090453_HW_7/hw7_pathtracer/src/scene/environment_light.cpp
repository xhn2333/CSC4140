...
void EnvironmentLight::init() {
    ...
    for (int j = 0; j < h; ++j) {
        for (int i = 0; i < w; ++i) {
            pdf_envmap[w * j + i] =
                envMap->data[w * j + i].illum() * sin(PI * (j + .5) / h);
    ...
    for (int j = 0; j < h; ++j) {
        ...
        for (int i = 1; i < w; ++i) {
            conds_y[j * w + i] =
                conds_y[j * w + i - 1] + pdf_envmap[j * w + i] / density[j];
        }
    }
...
Vector3D EnvironmentLight::bilerp(const Vector2D& xy) const {
    ...
    auto u0 = 1 - u1;
    return (envMap->data[top + left] * u1 + envMap->data[top + right] * u0) *
               v1 +
           (envMap->data[bottom + left] * u1 +
            envMap->data[bottom + right] * u0) *
               (1 - v1);
}
...
Vector3D EnvironmentLight::sample_L(const Vector3D p, Vector3D* wi,
    ...
    Vector2D s_xy = sampler_uniform2d.get_sample();
    auto y = std::upper_bound(marginal_y, marginal_y + envMap->h, s_xy.y) -
             marginal_y;
    auto x = std::upper_bound(conds_y + y * envMap->w,
                              conds_y + (y + 1) * envMap->w, s_xy.x) -
             (conds_y + y * envMap->w);
    *distToLight = INF_D;
